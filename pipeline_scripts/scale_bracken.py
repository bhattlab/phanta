...
def main():
    ###step 1- input, should be replaced with argv
    ...
    kraken_report =  sys.argv[3] #"/labs/asbhatt/mchakra/phaging/phage_abundance_pipeline/testing/classification/sample20.krak.report.filtering_decisions.txt"
    ...
    kraken_df = pd.read_csv(kraken_report, sep="\t")
    ...
    per_million_scaling = (tmp_merged_df['new_est_reads'].sum() / 1000000)
    tmp_merged_df['tmp_scaling'] = tmp_merged_df['fraction_total_reads']*(tmp_merged_df['median'].sum()/tmp_merged_df['median'])
    tmp_merged_df['community_abundnace'] = tmp_merged_df['tmp_scaling']/tmp_merged_df['tmp_scaling'].sum()
    tmp_merged_df['genetic_abundnace'] = tmp_merged_df['fraction_total_reads']
    tmp_merged_df['depth'] = (tmp_merged_df['new_est_reads']*read_length)/tmp_merged_df['median'] #which is estimation of copies
    tmp_merged_df['reads_per_million_bases'] = (tmp_merged_df['new_est_reads'] / (tmp_merged_df['median'] / 1000000)).round(4)
    tmp_merged_df['reads_per_million_reads'] = (tmp_merged_df['new_est_reads'] / per_million_scaling).round(4)
