#This script is necessary for creating a custom Phanta db. 
#Usage: python calculate_genome_size.py </path to database folder>
#The script will calculate geonome size per species and add the required files to the /library subfolder
...
def main():
    ...
    length_list=lengths.groupby('species_level_taxa')['Length'].apply(list).rename("lengths")
    max_len=lengths.groupby('species_level_taxa')['Length'].max().rename("max")
    mean_len=lengths.groupby('species_level_taxa')['Length'].mean().rename("mean")
    median_len=lengths.groupby('species_level_taxa')['Length'].median().rename("median")
    genome_count=lengths.groupby('species_level_taxa')['Length'].count().rename("count")
    taxa=lengths.groupby('species_level_taxa')['Taxonomy'].apply(list).rename("taxa_list")
    length_per_species = pd.concat([max_len,mean_len,median_len,genome_count,length_list,taxa],axis=1)
